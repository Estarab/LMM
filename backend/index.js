...
app.post('/api/submit-participant', async (req, res) => {
  ...
  try {
    ...
    const existingUser = await User.findOne({ phone });
    if (!existingUser) {
      return res.status(400).json({ message: 'Phone number not found in the users database. Please enter a valid phone number.' });
    }

    // Step 2: Check if the combination of phone and surname already exists in the participants collection
    const existingParticipant = await Participant.findOne({ phone, surname });
    if (existingParticipant) {
      return res.status(400).json({ message: 'This phone number and surname combination has already been used to pick a participant.' });
    }

    // Step 3: Create a new participant if all checks pass
