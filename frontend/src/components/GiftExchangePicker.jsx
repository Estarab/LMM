// src/components/GiftExchangePicker.js
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';

function GiftExchangePicker() {
  ...
  useEffect(() => {
    ...
    const checkSignup = async () => {
      try {
        ...
        setIsSignedUp(false);
        navigate('/signup'); // Redirect to sign-up if user is not signed up
      }
    };
  ...
  return isSignedUp ? (
    <div>
      {/* Gift Exchange Picker Component */}
      <h1>Gift Exchange Picker</h1>
    </div>
  ) : (
    <div>Please sign up to access the Gift Exchange Picker.</div>
  );
}
...
//   const [submittedPhones, setSubmittedPhones] = useState(new Set());
//   const [submittedSurnames, setSubmittedSurnames] = useState(new Set());
//   const [disabledButtons, setDisabledButtons] = useState(new Set()); // Track which buttons should be disabled
//   const navigate = useNavigate();
...
//     if (submittedPhones.has(userDetails.phone) && submittedSurnames.has(userDetails.surname)) {
//       alert('This phone number and surname combination has already been used!');
//       return;
//     }
...
//       setDisabledButtons((prev) => new Set(prev.add(selectedNumber)));
//     } catch (error) {
//       alert(error.response?.data?.message || 'Error submitting participant details');
//     }
//   };
...
//               key={i}
//               onClick={() => handleButtonClick(i + 1)}
//               disabled={disabledButtons.has(i + 1)} // Disable button if it's in the disabled set
//               className={`bg-red-600 text-white p-4 rounded-xl hover:bg-green-600 transform transition duration-300 ${disabledButtons.has(i + 1) ? 'bg-gray-400 text-white cursor-not-allowed' : ''}`}
//             >
//               {disabledButtons.has(i + 1) ? 'Already Picked' : i + 1}
//             </button>
//           ) : (
//             <button
//               key={i}
...
//         <a href="/user-details" className="text-green-600 text-xl">Click here to enter your details (Name and Phone Number)</a>
//       </div> */}
...
//               key={i}
//               onClick={() => handleButtonClick(i + 1)}
//               className="bg-red-600 text-white p-4 rounded-xl hover:bg-green-600 transform transition duration-300"
//             >
//               {i + 1}
//             </button>
//           ) : (
